...
function ThinkElectricSection() {
  ...
  const thinkContent = `
    ...
    and all-terrain fork lifts in different size configurations that are 100% electric. Our entire range is designed for zero emission, low noise operation without compromising speed, power 
    or operational length. Powered by BYD® Lithium Blade Battery technology our range of equipment is available in Australia by Enloq Loaders®, Australia.
  `;
...
function EnloqLoadersSection() {
  ...
  const enloqContent = `
    Enloq Loaders® focuses on the development and implementation of electric equipment for 
    industrial applications in support of a greener efficient work practices. Our goal is to design and produce specialty industrial EV equipment, with a focus on safety, 
    design, and reliability of the highest quality using the most advanced battery technology while 
    always maintaining an unparalleled value proposition.</br></br>

    Take a closer look at <span class="highlight-text">Enloq Loaders®</span> to see how technology can drive profits by eliminating 
    emissions, lower maintenance cost and reduce hourly running expenses. It's a greener process 
    requiring minimal energy and delivers massive carbon offsets. Contact any of our friendly staff to find out more or schedule a demonstration.
  `;
